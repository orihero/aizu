...
from .human import triangular_pick
...
def _enforce_daytime_default() -> bool:
    ...
    ``AIZU_IGNORE_DAYTIME=1`` to opt OUT for off-hours TESTING only — this lets
    ...
    """
    return os.environ.get("AIZU_IGNORE_DAYTIME", "").strip().lower() not in (
        "1", "true", "yes", "on")
...
class Pacer:
    ...
    def dwell(self) -> float:
        # Triangular (not flat-uniform) draw — flat uniform is a bot fingerprint
        # (see core/human.py). Mean is unchanged, so caps/ramps still hold.
        t = triangular_pick(self.rng, self.cfg.dwell_min, self.cfg.dwell_max)
        self._sleep(t)
        return t

    def between_reels(self) -> float:
        t = triangular_pick(self.rng, self.cfg.between_min, self.cfg.between_max)
        self._sleep(t)
        return t
