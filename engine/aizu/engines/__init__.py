"""aizu.engines — one self-contained engine per social network.
...
``aizu.core`` for shared transport (Router), persistence (Store), brief
parsing (Campaign), and the FeedSource content interface — never on each other.

``aizu.dispatch`` selects the engine for a run by ``campaign.platform`` and
