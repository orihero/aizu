...
class SessionConfig:
    ...
    # Outer wall-clock backstop for one post's processing block (open_reel + reply/
    # quote fetch). On breach the post is SKIPPED (warn) and the walk continues —
    # NEVER halt. Behind the per-call Playwright timeouts as defense in depth.
...
class XSession:
    def __init__(self, *, store: Store, router: Router, feed: FeedSource,
        ...
        self.cascade = XCascade(router, campaign, session_id=self.session_id)
    ...
    def _run_after_start(self, cid: str, plat: str) -> dict:
        ...
        try:
            for reel in self.feed.walk():
                ...
                # expensive block (vision, open_reel nav, reply/quote fetch) is bounded.
                ...
                # Wall-clock backstop before the expensive open_reel + reply block.
