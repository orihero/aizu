...
# identical to the instagram and x engines so one panel/ops query covers all three.
...
class _HeartbeatRouter:
    ...
    a watchdog exists. Kept identical in the instagram and x engines.
...
class LinkedInSession:
    def __init__(self, *, store: Store, router: Router, feed: FeedSource,
        ...
        self.cascade = LinkedInCascade(_HeartbeatRouter(router, self._touch),
    ...
    def _flag_relevant_discard(self, reel: Reel, reason: str) -> None:
        """FIX P0: record that a post which PASSED relevance never reached comment
        ...
        self.store.raise_flag(
            ...
            f"post {reel.reel_id}: passed relevance but was skipped before comment "
    ...
    def _reel_deadline_breached(self, reel: Reel, started: float) -> bool:
        """True once one post's BROWSER block (open_reel + comment fetch) has exceeded
        ...
        discarded relevant posts before FIX P0. The outer wall-clock backstop behind
        the per-call Playwright timeouts. On breach it emits a warn step; the caller
        SKIPS the remaining browser work and continues — NEVER halts."""
            ...
            for reel in self.feed.walk():
                ...
                if gate.relevant:
                    ...
                    if not opened:
                        ...
                        # for the comment fetch + scoring. Keeps the backstop's
