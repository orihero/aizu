"""YouTube decision cascade — text-only relevance + comment matching.

Standalone (NOT the Instagram cascade): the Data API exposes no video frames, so
there is no vision tier and ``classify_image`` is never called. Relevance is
judged on the video's title + description; a comment is matched with the video as
context ("VIDEO BEING COMMENTED ON"). Escalate-if-unsure re-runs the text call.
...
from .prompts import YT_MATCH, YT_RELEVANCE
...
def _comment_content(comment: Comment, reel: Optional[Reel]) -> str:
    """Match-stage content: the comment, plus the video it replies to.

    The video context (title + description) lets the extractor fill fields the
    bare comment omits — the commenter's intent is still judged from the comment.
    """
    if reel is None or not reel.caption:
        return comment.text
    parts = ["VIDEO BEING COMMENTED ON (posted by the channel):"]
    if reel.author:
        parts.append(f"channel: {reel.author}")
    if reel.caption:
        parts.append(f"title + description: {reel.caption}")
    parts.append(f"\nCOMMENT TO JUDGE:\n{comment.text}")
    return "\n".join(parts)
...
def _match_instruction(campaign: Campaign, fields: list[str]) -> str:
    ...
    if fields:
        ...
        instr += (
            ...
            "(or the video context when it legitimately applies), else use null — "
...
class YouTubeCascade:
    ...
    def gate_video(self, reel: Reel) -> RelevanceResult:
        """Relevance on title + description (text only; no frames on the Data API)."""
        cid = self.campaign.campaign_id
        system = self.campaign.relevance_prompt or YT_RELEVANCE
        ...
        log.debug("yt relevance video=%s relevant=%s score=%.2f conf=%.2f esc=%s",
    ...
    def score_comment(self, comment: Comment,
        ...
        system = self.campaign.match_prompt or YT_MATCH
        d = self.router.classify_text(
        ...
        log.debug("yt match comment=%s match=%s score=%.2f esc=%s",
