"""Telegram decision cascade — text-only relevance + reply matching.

Standalone (NOT the Instagram cascade): Telegram is text-first, so there is no
vision tier and ``classify_image`` is never called. Relevance is judged on the
channel message's text; a discussion reply is matched with the message as context
("CHANNEL MESSAGE BEING REPLIED TO"). Escalate-if-unsure re-runs the text call.

Per the locked per-engine-scoring decision, the small escalate/coerce helpers are
intentionally local rather than shared across engines.
"""
...
from .prompts import TG_MATCH, TG_RELEVANCE
...
def _comment_content(comment: Comment, reel: Optional[Reel]) -> str:
    """Match-stage content: the reply, plus the channel message it answers.

    The message context lets the extractor fill fields the bare reply omits — the
    replier's intent is still judged from the reply text.
    """
    if reel is None or not reel.caption:
        return comment.text
    parts = ["CHANNEL MESSAGE BEING REPLIED TO (posted by the channel):"]
    if reel.author:
        parts.append(f"channel: {reel.author}")
    if reel.caption:
        parts.append(f"message: {reel.caption}")
    parts.append(f"\nREPLY TO JUDGE:\n{comment.text}")
    return "\n".join(parts)
...
def _match_instruction(campaign: Campaign, fields: list[str]) -> str:
    ...
    if fields:
        ...
        instr += (
            ...
            "contain EXACTLY these keys and no others. Fill a key from the reply "
            "(or the message context when it legitimately applies), else use null — "
...
class TelegramCascade:
    ...
    def gate_message(self, reel: Reel) -> RelevanceResult:
        """Relevance on the channel message text (text only; no media in v1)."""
        cid = self.campaign.campaign_id
        system = self.campaign.relevance_prompt or TG_RELEVANCE
        ...
        log.debug("tg relevance msg=%s relevant=%s score=%.2f conf=%.2f esc=%s",
    ...
    def score_comment(self, comment: Comment,
        ...
        system = self.campaign.match_prompt or TG_MATCH
        d = self.router.classify_text(
        ...
        log.debug("tg match reply=%s match=%s score=%.2f esc=%s",
