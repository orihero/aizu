...
SQLite remains the durable record of what a run did. Stdlib only — no web framework.
  ...
  python -m reelradar.cli panel --db reelradar.db --panel-dir ../admin-panel/dist
"""
...
from .core.router import OpenRouterRouter, _parse_csv_env
...
PLATFORM_ADMINS_ENV = "REELRADAR_PLATFORM_ADMINS"
...
WORKER_BOOTSTRAP_ENV = "REELRADAR_WORKER_BOOTSTRAP_TOKEN"
...
MIN_AGENT_VERSION_ENV = "REELRADAR_MIN_AGENT_VERSION"
...
class PanelHandler(SimpleHTTPRequestHandler):
    ...
    def _client_ip(self) -> str:
        ...
        plane runs behind a reverse proxy, set REELRADAR_TRUSTED_PROXIES to the proxy's
    ...
    def _handle_generate_campaign(self, payload: Any) -> None:
        ...
        store = Store(self.db_path)
        try:
            router = OpenRouterRouter(store=store, spend_cap_usd=GENERATE_SPEND_CAP_USD)
        ...
        try:
            ...
            if secret is not None:
                ...
                store.record_audit(org_id, actor["id"], "integration_disconnected",
                                   target=platform)
        except SecretCipherError as e:        # no/invalid REELRADAR_SECRET_KEY
            self._send_json(500, False, error=str(e))
            return
        except Exception as e:  # noqa: BLE001
            self._send_json(500, False, error=str(e))
    ...
    def _handle_worker_register(self, payload: Any) -> None:
        ...
        new box presents the shared REELRADAR_WORKER_BOOTSTRAP_TOKEN. Either way the
        ...
        try:
            ...
            if admin is not None and not disabled:
                try:
                    ...
                    # Misconfigured/absent REELRADAR_SECRET_KEY → 500, never a silent
    ...
    def _handle_admin_fleet(self) -> None:
        ...
        # session), replacing the interim REELRADAR_PLATFORM_ADMINS env allowlist.
...
def serve(db_path: str, panel_dir: str, config_dir: str,
    ...
    if run_manager is None:
        # engine_root (parent of the reelradar package) is the cwd for `-m
        # reelradar.cli`; per-run logs land under engine/run-logs/.
