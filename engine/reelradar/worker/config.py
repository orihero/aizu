...
reuses ``cli._run_session_loop`` verbatim (the same surface a CLI ``reelradar run``
...
def _parse_capabilities_env() -> tuple:
    ...
    bad config must not stop the worker from registering):
      ...
      1. ``REELRADAR_WORKER_CAPABILITIES`` — JSON array of ``[org, platform, handle]``
         (explicit; org/handle may be null). Entries whose platform is unsupported are
         dropped.
      2. ``REELRADAR_WORKER_PLATFORMS`` — comma-separated platform list, or ``all``/``*``
    ...
    raw = os.environ.get("REELRADAR_WORKER_CAPABILITIES")
    if raw:
        try:
    ...
    plats_raw = os.environ.get("REELRADAR_WORKER_PLATFORMS", "").strip()
    if not plats_raw:
        return ()
...
class WorkerConfig:
    ...
    capabilities: tuple = ()
    # Shared first-register secret (REELRADAR_WORKER_BOOTSTRAP_TOKEN). Presented as the
    ...
    def from_env(cls, *, dispatch_base_url: Optional[str] = None) -> "WorkerConfig":
        ...
        ``REELRADAR_DISPATCH_URL`` is required (or pass it explicitly) — the worker
        cannot run without a dispatch endpoint to pull from.
        """
        url = dispatch_base_url or os.environ.get("REELRADAR_DISPATCH_URL")
        if not url:
            raise ValueError(
                "REELRADAR_DISPATCH_URL not set — the worker needs a dispatch base URL "
                "to lease jobs from (e.g. https://cloud.example.com or the local stub)")
        cfg_dir = Path(os.environ.get("REELRADAR_CONFIG", "config"))
        db_path = os.environ.get("REELRADAR_DB", "reelradar.db")
        state_dir = Path(os.environ.get("REELRADAR_WORKER_STATE", ".worker-state"))
        cs_enabled = _env_flag("REELRADAR_CONTROL_SURFACE")
        cs_token = os.environ.get("REELRADAR_CONTROL_TOKEN") or None
        if cs_enabled and not cs_token:
            ...
            raise ValueError(
                "REELRADAR_CONTROL_SURFACE is enabled but REELRADAR_CONTROL_TOKEN is not "
        ...
        return cls(
            ...
            db_path=db_path,
            state_dir=state_dir,
            cdp_url=os.environ.get("REELRADAR_CDP_URL", "http://127.0.0.1:9222"),
            spend_cap=float(os.environ.get("REELRADAR_SPEND_CAP", "20.0")),
            text_model=os.environ.get("OPENROUTER_TEXT_MODEL", "openrouter/owl-alpha"),
            vision_model=os.environ.get(
                "OPENROUTER_VISION_MODEL", "nex-agi/nex-n2-pro:free"),
            bootstrap_token=os.environ.get("REELRADAR_WORKER_BOOTSTRAP_TOKEN") or None,
            stall_timeout_sec=float(os.environ.get(
                "REELRADAR_STALL_TIMEOUT_SEC", DEFAULT_STALL_TIMEOUT_SEC)),
            capabilities=_parse_capabilities_env(),
            control_surface_enabled=cs_enabled,
            control_surface_port=int(os.environ.get(
                "REELRADAR_CONTROL_PORT", DEFAULT_CONTROL_SURFACE_PORT)),
