...
log = get_logger("reelradar.worker.single_flight")
...
def _pid_alive(pid: int) -> bool:
    ...
    check: it succeeds for a live process we own, raises ``ProcessLookupError``
    (ESRCH) when the process is gone, and ``PermissionError`` (EPERM) when it
    exists but is owned by another user. A dead pid is the only case we treat as
    not-alive, so a leaked lock whose owner is EPERM is (correctly) kept.
    """
    try:
        os.kill(pid, 0)
        ...
        # Malformed pid (e.g. 0/negative) or unexpected OS error: assume alive so
        # we never false-reclaim; the age backstop still applies.
        return True
    return True
