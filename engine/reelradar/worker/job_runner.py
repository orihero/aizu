...
Two layers live here:
  ...
  - ``_execute_job`` — the in-process body: resolve the brief + soul, stamp a run_id,
    drive ``cli._run_session_loop`` (the exact CLI ``reelradar run`` seam), clean up the
  ...
  - ``run_one_job`` — the Phase-6 SUPERVISOR: it launches ``python -m
    reelradar.worker.job_child`` as a killable child, then polls the child while watching
...
log = get_logger("reelradar.worker.job_runner")
...
def run_one_job(store, job: JobSpec, *, cfg: WorkerConfig, halt: threading.Event,
    ...
    try:
        ...
        proc = popen(
            [sys.executable, "-m", "reelradar.worker.job_child", "--spec-file",
             str(spec_path)],
...
def _child_env(run_id: str, cfg: WorkerConfig) -> dict:
    """The child inherits the sidecar's FULL environment (it needs REELRADAR_SECRET_KEY /
    OPENROUTER_API_KEY etc. for a live run) plus REELRADAR_RUN_ID (so the child's own
    ...
    return {
        **os.environ,
        "REELRADAR_RUN_ID": run_id,
        "REELRADAR_LOG_FILE": str(cfg.state_dir / _JOB_LOG_DIRNAME / "reelradar.log"),
    }
...
def _run_env(run_id: str, pause_path: Path) -> Iterator[None]:
    """Stamp REELRADAR_RUN_ID (so Session._emit streams run_events for this run) and
    REELRADAR_PAUSE_FILE (the cooperative-pause sentinel), restoring the prior values
    ...
    if not _RUN_ENV_GUARD.acquire(blocking=False):
        raise RuntimeError(
            ...
            "subprocess boundary")
    prev_run_id = os.environ.get("REELRADAR_RUN_ID")
    prev_pause = os.environ.get("REELRADAR_PAUSE_FILE")
    os.environ["REELRADAR_RUN_ID"] = run_id
    os.environ["REELRADAR_PAUSE_FILE"] = str(pause_path)
    try:
        yield
    finally:
        _restore_env("REELRADAR_RUN_ID", prev_run_id)
        _restore_env("REELRADAR_PAUSE_FILE", prev_pause)
...
def _pid_alive(pid: int) -> bool:
    """True if a process with ``pid`` currently exists. Signal 0 probes without killing."""
    if pid <= 0:
        return False
    try:
        os.kill(pid, 0)
