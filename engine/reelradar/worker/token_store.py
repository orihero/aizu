...
one backend, resolved ONCE at construction from ``REELRADAR_TOKEN_BACKEND``:
  ...
  - ``auto``    — ALWAYS the Fernet file (default). Keyring is strictly OPT-IN because an
    ...
    set ``REELRADAR_TOKEN_BACKEND=keyring`` on a box whose keychain is pre-authorized.
...
log = get_logger("reelradar.worker.token_store")

_TOKEN_BACKEND_ENV = "REELRADAR_TOKEN_BACKEND"
_KEYRING_SERVICE = "reelradar-worker-token"
...
_PROBE_TOKEN = "reelradar-keyring-probe"
...
def resolve_backend_kind(env_value: Optional[str], *, keyring_available: bool,
    ...
    if choice == "keyring":
        ...
        raise KeyringBackendError(
            "REELRADAR_TOKEN_BACKEND=keyring but no working OS keychain is available")
