        ...
        if ok:
            if action_type == "like":
                ...
                self._emit("engage", "success", f"Followed @{target}",
                           {"target": target})
        # An action-block is a hard stop (soul.md), never retried.
        if self.feed.detect_action_block():
            raise HaltSession("action-block detected after " + action_type,
                              kind="action_block")
    ...
    def _process_comments(self, reel: "Reel") -> int:
        ...
        if not self.feed.healthy():
            ...
            if self._empty_streak >= self.cfg.empty_interception_halt:
                raise HaltSession("empty interception for N reels — endpoint drift",
                                  kind="canary")
        else:
            self._empty_streak = 0
    ...
    def _run_after_start(self, cid: str, plat: str) -> dict:
        ...
        try:
            for reel in self.feed.walk():
                ...
                try:
                    # Frames captured lazily (only if the gate needs vision).
