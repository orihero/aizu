...
VALID_STATUS = {"new", "in_progress", "interested", "closed", "couldnt_connect", "archived"}
...
-- decryption is keyed by REELRADAR_SECRET_KEY (see reelradar.secrets).
...
-- Campaign from it and `reelradar run --campaign <id>` executes it instead of the
...
-- blob (reelradar.secrets), NEVER plaintext.
...
class Store:
    def __init__(self, db_path: str | Path, secret_cipher: Optional["SecretCipher"] = None):
        ...
        # Lazily built from REELRADAR_SECRET_KEY on first secret use unless injected
    ...
    def upsert_match(self, *, campaign_id: str, reel_id: str, comment_id: str,
                     ...
                     found_by_models: Optional[list[str]] = None) -> None:
    ...
    def _upsert_match_row(self, c: sqlite3.Cursor, *, campaign_id: str, reel_id: str,
        ...
        human-set status); `session_id`/`captured_at` record first-capture provenance."""
        now = time.time()
        ...
        c.execute(
            ...
            (campaign_id, org_id, platform, reel_id, comment_id, session_id, username,
             text, lang, score, reason, blob, tier, found_by_blob, cap, now),
        )
        ...
        with self._tx() as c:
            c.execute(
                """INSERT OR REPLACE INTO sessions(session_id, campaign_id, platform,
                   ...
                   VALUES(?,?,?,?, 'running', ?, ?, ?, ?)""",
                (session_id, campaign_id, platform, time.time(), run_id, org_id,
                 engine_mode, account_id),
            )
        ...
        with self._tx() as c:
            c.execute(
                """UPDATE sessions SET
                     ...
                     comments_scored=?, matches=?, escalations=?, spend_usd=?,
                     feed_health_flag=?
                   WHERE session_id=?""",
                (counters.reels_seen, counters.already_seen_skips, counters.relevance_passes,
                 ...
                 counters.spend_usd, int(counters.feed_health_flag), session_id),
            )
    ...
    def _cipher(self) -> "SecretCipher":
        """The Fernet cipher, built once from REELRADAR_SECRET_KEY unless injected.
    ...
    def get_job_for_run(self, run_id: str,
        ...
        try:
            ...
            rows = self._conn.execute(
                "SELECT * FROM jobs WHERE org_id=? ORDER BY created_at DESC",
                (org_id,)).fetchall()
            for row in rows:
    ...
    def reconcile_orphan_sessions(self, *, now=None) -> int:
        """Close sessions stuck at status='running' (ended_at IS NULL) that no longer
        correspond to any live run. Called once at server startup: an in-process run
        can't survive a restart (RunManager's active slot is in-memory), so any such
        row is an orphan from a crashed/killed run. A session backing a still-in-flight
        fleet job IS excluded — but correlated by run_id (from the job spec), NOT by
        jobs.session_id: a fleet job populates session_id only at ack, so a mid-flight
        (never-acked) live run had NO protection under the old session_id join and a
        restart would wrongly halt it. We now exclude any session whose run_id belongs
        to a live (queued/leased/running, not dead-lettered) job. Returns the count
        reconciled. Write-locked via _tx_immediate."""
        now_v = now if now is not None else time.time()
    ...
    def create_platform_admin(self, *, email: str, password_hash: str,
        ...
        `mfa_secret` is a Fernet blob of the TOTP secret (reelradar.secrets) — NEVER
    ...
    def get_admin_totp_secret(self, admin_id: int) -> Optional[str]:
        ...
        SecretCipherError (caught at the API boundary) if REELRADAR_SECRET_KEY is
