...
corrupt the parse. So when ``REELRADAR_RUN_ID`` is set we attach **file handlers
...
_SECRET_ENV_VARS = ("OPENROUTER_API_KEY", "REELRADAR_SECRET_KEY", "TELEGRAM_API_HASH",
...
def _want_color(color: Optional[str]) -> Optional[bool]:
    ...
    choice = (color or os.environ.get("REELRADAR_LOG_COLOR") or "auto").strip().lower()
...
def _engine_root() -> Path:
    """The engine dir (parent of the ``reelradar`` package) — where ``logs/`` lives."""
...
def _default_log_dir() -> Path:
    """Where ``reelradar.log`` lives when no explicit path is given.

    A frozen (PyInstaller) build runs from inside a READ-ONLY, code-signed app
    bundle — the ``reelradar`` package sits at ``.../Resources/sidecar/.../_internal/
    reelradar``. Defaulting to ``_engine_root()/logs`` there would write *inside* the
    ...
    if getattr(sys, "frozen", False):
        state = os.environ.get("REELRADAR_WORKER_STATE")
...
def _resolve_log_path(log_file: Optional[str]) -> Optional[Path]:
    ...
    value = log_file if log_file is not None else os.environ.get("REELRADAR_LOG_FILE")
    if value and value.strip().lower() == "off":
        return None
    if value:
        return Path(value).expanduser()
    return _default_log_dir() / "reelradar.log"
...
def configure_logging(*, level: Optional[object] = None,
    ...
    """Configure the ``reelradar`` logger tree. Idempotent unless ``force=True``.

    - Console (Rich, colorful) → stderr, at ``level`` — UNLESS this process is a
      spawned panel run (``REELRADAR_RUN_ID`` set), where it is omitted so the
      stdout/stderr stream the panel parses stays pristine.
    - Rotating file → ``engine/logs/reelradar.log`` at ``file_level`` (default
      DEBUG so the archive is complete), unless ``REELRADAR_LOG_FILE=off``.
    ...
    console_level = _coerce_level(level, _coerce_level(
        os.environ.get("REELRADAR_LOG_LEVEL"), _DEFAULT_LEVEL))
    file_lvl = _coerce_level(file_level, _coerce_level(
        os.environ.get("REELRADAR_LOG_FILE_LEVEL"), _DEFAULT_FILE_LEVEL))
    run_id = os.environ.get("REELRADAR_RUN_ID")
...
def run_log_path(run_id: str, *, log_file: Optional[str] = None) -> Optional[Path]:
    ...
    ``REELRADAR_RUN_ID`` — ``<logdir>/run-<run_id>.log``. Single source of truth so the
    ...
    when file logging is disabled (``REELRADAR_LOG_FILE=off``)."""
...
def get_logger(name: str) -> logging.Logger:
    """A namespaced logger. Pass ``__name__`` (already ``reelradar.*``).

    An entry point run as ``python -m reelradar.<mod>`` sees ``__name__ ==
    "__main__"``, which would fall outside the configured ``reelradar`` tree and
