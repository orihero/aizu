...
- **TOTP MFA** (RFC 6238, SHA-1, 6 digits, 30s step) — a second factor on every admin
  ...
  provisioning URI, and stored Fernet-encrypted at rest (reelradar.secrets).
...
`reelradar.auth` — the admin plane must not fork crypto primitives. Only the
...
ADMIN_IP_ALLOWLIST_ENV = "REELRADAR_ADMIN_IP_ALLOWLIST"
...
ADMIN_TRUSTED_PROXIES_ENV = "REELRADAR_TRUSTED_PROXIES"
