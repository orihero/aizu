...
lifecycle:
  ...
  - `build_argv`     pure mapping from a RunSpec to a `reelradar` CLI argv.
...
def build_argv(spec: RunSpec, python_exe: str, db_path: str,
    ...
    """Pure RunSpec → `reelradar` CLI argv.
    ...
    """
    argv = [python_exe, "-m", "reelradar.cli", "--db", db_path]
    if spec.scope == "all":
...
class RunManager:
    ...
    def _child_env(self, run_id: str, spec: RunSpec) -> dict:
        ...
        env = dict(os.environ)
        env["REELRADAR_RUN_ID"] = run_id
        if spec.org_id is not None:
            env["REELRADAR_ORG_ID"] = str(spec.org_id)
        ...
        env["REELRADAR_PAUSE_FILE"] = str(self._pause_path(run_id))
        return env
