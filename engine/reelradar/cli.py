"""Command-line entrypoint.
  ...
  reelradar run   --dry-run          run the full loop on the built-in fake feed
  reelradar run                      live: OpenRouter + CDP attach (CDP not yet wired)
  reelradar status                   print open health flags + last session

Config paths default to ./config/soul.md and ./config/campaign.md; DB defaults
to ./reelradar.db. The panel reads the same DB.
"""
...
from .core.router import OpenRouterRouter, env_flag, _parse_csv_env
from .core.store import Store

# Explicit name (not __name__): run as `python -m reelradar.cli`, __name__ is
# "__main__", which would sit outside the configured `reelradar` logger tree.
log = get_logger("reelradar.cli")
...
def _build_run_io(campaign: Campaign, store: Store, dry_run: bool,
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise RuntimeError("OPENROUTER_API_KEY not set (required for live run)")
    ...
    router = OpenRouterRouter(store=store, spend_cap_usd=args.spend_cap,
    ...
    if not pacer.cfg.enforce_daytime:
        log.warning("Daytime guard DISABLED (REELRADAR_IGNORE_DAYTIME) — live run will "
...
def _build_warming_io(campaign: Campaign, store: Store, args: argparse.Namespace):
    ...
    REELRADAR_IGNORE_DAYTIME env opt-out (which live runs set), so warming can
...
def _run_one(*, campaign: Campaign, store: Store, soul, dry_run: bool,
    ...
    try:
        return dispatch.run_engine_session(
            ...
            pacer=pacer, run_id=os.environ.get("REELRADAR_RUN_ID"),
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="reelradar", description="Reel-Comment Discovery Agent")
    ...
    def _add_run_args(sp: argparse.ArgumentParser) -> None:
        ...
        sp.add_argument("--cdp-url",
                        default=os.environ.get("REELRADAR_CDP_URL", "http://127.0.0.1:9222"))
