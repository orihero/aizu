"""FIX P0 (X): the per-post wall-clock budget bounds the BROWSER block, not the
cascade.
...
from aizu.engines.x.session import (
    RELEVANT_REEL_DISCARDED_FLAG, SessionConfig, XSession)
...
def _relevant_post(post_id: str) -> Reel:
    return Reel(post_id, caption="acme", author="acmehq",
                comments=[Comment(f"c-{post_id}", f"u-{post_id}", "pricing?")])


def test_post_whose_classification_overruns_the_budget_still_reaches_reply_scoring():
    store, _path = _store()
    clock = _Clock()
    # 100s > the 90s default budget, spent entirely inside the relevance gate.
    out = XSession(store=store, router=SlowGateRouter(clock, gate_seconds=100.0),
...
def test_relevance_passes_never_disagrees_with_the_persisted_seen_reels_verdict():
    ...
    posts = [_relevant_post("hit"),
             Reel("miss", caption="cat memes", author="pets", comments=[])]
    out = XSession(store=store, router=SlowGateRouter(clock, gate_seconds=100.0),
                   feed=FakeFeed(posts), soul=None, campaign=_campaign(),
                   pacer=_pacer(), cfg=SessionConfig(), clock=clock).run()
...
def test_browser_work_that_burns_the_budget_skips_replies_and_flags_the_lost_post():
    store, _path = _store()
    clock = _Clock()
    feed = SlowOpenFeed([_relevant_post("wedged")], clock, open_seconds=100.0)
    out = XSession(store=store, router=SlowGateRouter(clock, gate_seconds=0.0),
...
def test_no_flag_and_no_skip_when_the_post_stays_under_the_budget():
    store, _path = _store()
    clock = _Clock()
    out = XSession(store=store, router=SlowGateRouter(clock, gate_seconds=1.0),
