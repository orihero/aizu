...
aizu/config.py     loaders for soul.md + campaign.md
aizu/store.py      SQLite (WAL): matches, state, counters, flags, spend
aizu/router.py     model router — OpenRouter cloud tier + spend/degrade
aizu/mock_router.py deterministic offline router (dry runs + tests)
aizu/cascade.py    relevance gate + comment match (escalate-if-unsure)
aizu/feed.py       FeedSource interface · FakeFeed
aizu/parsers.py    pure shape-driven reel/comment extractors (tested)
aizu/cdp.py        CDPFeed — live Playwright-over-CDP attach + interception
aizu/pacing.py     human-paced, daytime-only timing
aizu/session.py    orchestrator + three-tier failure handling
aizu/panel.py      DB → /api/state adapter for the admin panel
aizu/server.py     local bridge server: /api/state + write/control endpoints
aizu/runner.py     RunManager — spawns + tracks panel-triggered engine runs
aizu/cli.py        `aizu run [--campaign] [--dry-run]` · `run-all` · `status` · `panel`
...
python -m aizu.cli --db aizu.db run --dry-run    # full loop, no network
python -m aizu.cli --db aizu.db run-all --dry-run # every live campaign, sequentially
python -m aizu.cli --db aizu.db status           # open health flags
python -m aizu.cli --db aizu.db panel            # serve admin panel at :8765

# live (needs key; CDP attach not yet wired — see below)
export OPENROUTER_API_KEY=...
python -m aizu.cli run                       # runs config/campaign.md
python -m aizu.cli run --campaign <id>       # runs a panel-authored brief (DB)
python -m aizu.cli run-all                   # runs every status=live campaign
```
...
(`sessions`) remains the durable record. See `aizu/runner.py`.
...
`aizu panel` serves the mockup in `../admin-mockup` and injects a generated
...
# start the warmed Chrome with remote debugging (port 9333, matches AIZU_CDP_URL):
...
#     --remote-debugging-port=9333 --user-data-dir="$HOME/.aizu-chrome-profile")
export OPENROUTER_API_KEY=...
python -m aizu.cli run        # attaches, walks the reels feed, scores comments
```
