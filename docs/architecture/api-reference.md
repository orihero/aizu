...
- **An omitted `durationMinutes` means "bounded by the lead target", not "unbounded".** A run is defined by the leads it must FIND, not by how long it may look: the panel's launch form asks that one question and sends no duration, and `cli._run_one_channel` then loops discovery sessions until the target is met. The spec keeps `duration_minutes=None`; each execution path applies its own runaway guard behind the target, because that loop's only other exits are a halt and the daytime window and an algorithmic feed that never matches would otherwise spin for days. In-process: `runner.build_argv` appends `TARGET_RUN_GUARD_MINUTES` (720 — the API's own ceiling, and the width of the daytime write window, so a run started inside the window can never hit it first) whenever a campaign run has a target and no duration. Fleet: the box's `max_job_minutes` (default 90) already fills in for an absent duration, and it is deliberately NOT overridden here — the same number is the worker's supervisor deadline.
...
- **One lead target per RUN, not per channel.** A multi-platform campaign fans out one session per channel (`cli._run_session_loop`), and each channel is handed what the run still NEEDS — `target − matches banked so far` — with the fan-out breaking as soon as the target is met. Before this every channel got its own copy of the budget, so a 3-channel campaign could deliver 3× a target the org is metered on. The engine's summary carries `stop_reason` (`target_met | time_cap | halt | single_pass | done`), which `runner._summarize` folds into the run-history line, so a run that ends SHORT says whether its sources were swept (`single_pass` — deterministic YouTube/Reddit/Telegram seeds are swept once per run by design) or the runaway guard fired.
