...
|---|---|
| `store.py` lives at `engine/reelradar/store.py` (§3 header, §3.2) | **`engine/reelradar/core/store.py`** — shared cross-cutting modules (`store.py`, `logsetup.py`) live under `core/`. |
| Server/panel/rbac under `engine/reelradar/` (implied generic) | Confirmed at **package root**: `engine/reelradar/{server.py, panel.py, panel_org.py, rbac.py, secrets.py}`. `billing.py` correctly belongs here at the root (panel/server cross-cut), **not** under `core/` and **not** under `engines/{platform}/`. |
...
| Prior claim | Corrected value |
|---|---|
| Polar token/secret "covered by existing RedactingFilter; add patterns if needed" | **Not covered today.** `core/logsetup.py`: `_SECRET_ENV_VARS` (line 102) = `('OPENROUTER_API_KEY','REELRADAR_SECRET_KEY','TELEGRAM_API_HASH')`; `RedactingFilter` at 115–138 snapshots these at `configure_logging()` time. **Add `POLAR_ACCESS_TOKEN` and `POLAR_WEBHOOK_SECRET`** to `_SECRET_ENV_VARS`. |
...
ReelRadar/AIZU needs paid subscriptions, but **Stripe does not onboard Uzbekistan-based
...
- Acts as the legal seller, so **Polar** calculates, collects, and remits global sales tax /
  VAT — ReelRadar never registers for tax in foreign jurisdictions.
...
> **ReelRadar (the vendor) has ONE Polar account.** Each customer org is a *customer* inside
> that single Polar account.

Therefore:

- **Polar credentials are platform-level**, configured via **environment variables** (same tier
  as `REELRADAR_SECRET_KEY`) — *not* per-org, *not* in `integration_secrets`.
...
The panel backend is a **stdlib-only** `ThreadingHTTPServer` (`engine/reelradar/server.py`) over
SQLite (`engine/reelradar/core/store.py`), with a `{ok, data, error}` JSON envelope,
...
In `engine/reelradar/billing.py`, define a `BillingProvider` Protocol; `PolarClient` is its
...
`engine/reelradar/billing.py` *(new)* · `core/store.py` · `server.py` · `panel.py` ·
...
> `REELRADAR_SECRET_KEY` already exists and is unrelated — Polar creds are separate env vars.
