...
**Architecture overview:** `engine/docs/ENGINES.md` (updated; read §0, §5, §6 first)
...
- **Shared CDP harness** — `engine/reelradar/core/cdp.py` (`CDPFeedBase` + `CDPBaseConfig`):
...
- **LinkedIn engine** — `engine/reelradar/engines/linkedin/` (`parsers.py`, `cdp.py`,
  `cascade.py`, `prompts.py`, `session.py`). Managed-CDP, copy-first, vision when copy thin,
  single comment surface, read-only.
- **X engine** — `engine/reelradar/engines/x/` (same file set). Managed-CDP, text-first, **two
...
- **Docs** — `engine/docs/ENGINES.md` (two new sections + 6-way table).
...
1. **Warm the accounts** (manual, weeks — see PRDs §10). LinkedIn: a complete, credible profile.
   ...
   single warmed Chrome the engine attaches to (`REELRADAR_CDP_URL`, default `:9222` — note the
...
1. **(Optional) Surface managed rows in Settings.** Today the server only emits `integration`
   ...
   (`engine/reelradar/server.py`, wherever the Instagram managed row is derived). The panel already
...
| Area | Path |
|---|---|
| Shared CDP base | `engine/reelradar/core/cdp.py` |
| LinkedIn engine | `engine/reelradar/engines/linkedin/{parsers,cdp,cascade,prompts,session}.py` |
| X engine | `engine/reelradar/engines/x/{parsers,cdp,cascade,prompts,session}.py` |
| Dispatch wiring | `engine/reelradar/dispatch.py` (`build_feed`, `select_engine`) |
| Platform list (engine) | `engine/reelradar/core/config.py::SUPPORTED_PLATFORMS` |
...
| Architecture doc | `engine/docs/ENGINES.md` |
...
(`soul.md` + `campaign.md` with `platform: <p>`): `python -m reelradar.cli --db /tmp/x.db run
--config <dir> --dry-run`.
```
```
