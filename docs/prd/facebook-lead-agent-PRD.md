# Facebook Post-Comment Discovery Agent — PRD (v1 & v2)
...
**Goals**
- Surface brief-matching comments from public posts, Reels, and Group threads, where relevance, goal type, what-to-extract, and threshold all live in `campaign.md`.
- Survive on Facebook — behave like a real human account, never trip automation enforcement.
...
- The bot never likes, reacts, follows, comments, joins, or DMs. Dwell + scroll only.
...
- **Feed steering is manual + recurring** — the operator seeds the feed by hand (joining relevant Groups, following Pages, watching the right content) during warming and re-nudges it periodically as it drifts (see §10).
...
- **campaign.md** — warming/seed direction (which Groups, Pages, topics), relevance definition, goal type, what to extract & score, threshold, language mix.
...
```
- **Engine:** Python + Playwright over CDP. Walks the feed/Groups, reads post text + on-screen text, intercepts JSON, runs the cascade against the active brief, writes matches + state.
...
|---|---|---|---|
| Relevance gate | is this post relevant to the brief? | Local text → **local vision (reads on-screen text of image/video)** → **escalate-if-unsure → cloud** | the harder, irreversible decision now has a backup |
| Match scoring | is this comment a match? | Local → escalate-if-unsure → cloud | |
| Vision / OCR | read price / project / text burned into images & video frames | Local (Qwen2.5-VL 7B class) — **v1** | image-heavy platform; load-on-demand, then unload |
| Audio (v2) | voiceover transcript (Reels/video posts) | Local (Whisper) | rare, runs last |
...
- **feed-health flag** — set when the already-seen-skip ratio crosses threshold (feed/Groups tapped out / drifting).
...
- `extracted` — brief-defined JSON (lead → phone/intent; partner → post author; signal → topic).
...
- Weeks 1–2: manual warming only, no automation. Facebook enforces real-name identity strictly — warm a credible, established profile.
- **Steering is manual + recurring.** The operator joins relevant Groups, follows the right Pages, and watches matching content by hand; the feed and Group surfacing both reward this. Expect a recurring few-minutes maintenance task, not one-time setup. A rising already-seen-skip ratio (§7) is the signal to go re-steer (join more Groups / widen Pages).
- Agent runs: ramp from low — ~1–2 sessions/day, 15–30 min, 20–40 posts; dwell 3–30s, between-post 2–8s, randomized; daytime only. Ramp until resistance, then hold below it. Caps discovered empirically.
...
- Manual account warming (established profile) + Group/Page seeding; ongoing re-steering.
- CDP attach to warmed Chrome.
- Feed + Groups discovery loop: dwell-on-relevant / scroll-past-irrelevant.
- **Relevance gate: post text → on-screen-text (vision/OCR) → escalate-if-unsure to cloud.**
...
- **Vision/OCR tier (reads on-screen text of images & video frames).**
...
- **Audio tier:** Whisper transcription of Reel/video voiceover.
...
## 12. Open questions
- Exact session/daily caps — discover empirically (Facebook restricts established accounts more gently than fresh ones).
- Local model sizes (text + vision both resident-capable?) — measure real free memory on the M4 first.
- The post + comment endpoints — identify once in DevTools; expect drift; Groups vs main-feed may differ.
...
| Risk | Mitigation |
|---|---|
| Account ban / restriction | read-only · established-profile warming · CDP-attach · human-paced ramp · halt-on-resistance |
| Real-name enforcement / identity checks | warm a credible established profile; halt + alert on any identity challenge, never auto-resolve |
| Endpoint drift breaks scraping | intercept by URL + response shape, not hardcoded IDs; empty-interception canary halts |
| Relevance judged half-blind (text only) | vision/OCR reads on-screen text of images & video **in v1** + escalate-if-unsure to cloud |
| Local model weak on Uzbek | cloud escalation on both gate and scoring + hand-labeled validation set |
| Feed / Groups run dry or drift off-niche | already-seen-skip counter → dashboard flag → operator joins more Groups / re-steers |
