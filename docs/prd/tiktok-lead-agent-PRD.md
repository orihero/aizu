# TikTok Video-Comment Discovery Agent — PRD (v1 & v2)
...
- Survive on TikTok — behave like a real human account, never trip automation enforcement (the most aggressive bot detection of the video platforms here).
...
- The bot never likes, follows, comments, or DMs. Watch + scroll only.
...
- **Feed steering is manual + mobile + recurring** — desktop watching does not meaningfully train the algorithm; the operator steers the For You feed by hand on mobile during warming and re-nudges it periodically as it drifts (see §10).
...
- **campaign.md** — warming/seed direction (which topics, hashtags, creators), relevance definition, goal type, what to extract & score, threshold, language mix.
...
```
- **Engine:** Python + Playwright over CDP. Walks the For You feed, reads caption + on-screen text + transcript, intercepts JSON, runs the cascade against the active brief, writes matches + state.
...
|---|---|---|---|
| Relevance gate | is this video relevant to the brief? | Local text (caption) → **local vision (on-screen text)** + **local transcript** → **escalate-if-unsure → cloud** | video platform; captions are thin, so vision **and** audio are central |
| Match scoring | is this comment a match? | Local → escalate-if-unsure → cloud | |
| Vision / OCR | read text burned into frames | Local (Qwen2.5-VL 7B class) — **v1** | load-on-demand, then unload |
| Audio / transcript | spoken voiceover transcript | Local (Whisper) — **v1, promoted** | TikTok captions are minimal; transcript often carries the meaning |

One model resident at a time (36GB cap). **Batch by stage** — run the text gate across videos, then load vision/audio once for the batch that needs it, unload, then escalate the unsure minority to cloud. Cloud tier needs fallback (retry queue or degrade-to-local + flag) and per-campaign spend logging.
...
- `extracted` — brief-defined JSON (lead → handle/intent; partner → video author; signal → topic).
...
- **Halt + alert human (stop session):** login expired, checkpoint/captcha/challenge, action-block, or empty interception for N videos (endpoint drift). The bot never attempts to resolve a checkpoint.

## 10. Pacing & feed steering
- Weeks 1–2: manual warming only, no automation. **TikTok runs the most aggressive bot detection of the video platforms here** — warm carefully and ramp slowly; signature/device fingerprinting is hostile to automation.
- **Steering is manual + mobile + recurring.** The For You feed is strongly behavioral and mobile-trained — the operator watches the right content by hand on the phone during warming and re-nudges periodically; desktop watching does not steer the feed. Expect a recurring few-minutes maintenance task, not one-time setup. A rising already-seen-skip ratio (§7) is the signal to go re-steer on mobile.
- Agent runs: ramp from very low — ~1–2 sessions/day, 15–25 min, 15–35 videos; dwell 3–25s, between-video 2–8s, randomized; daytime only. Ramp until resistance, then hold well below it. Caps discovered empirically and kept conservative.
...
- Manual account warming + 1–2 week mobile feed-training; ongoing mobile re-steering.
- CDP attach to warmed Chrome.
- For You discovery loop: watch-relevant / scroll-past-irrelevant.
- **Relevance gate: caption → on-screen-text (vision/OCR) + transcript (Whisper) → escalate-if-unsure to cloud.**
...
**v1 done =** account survives weeks unbanned · sessions complete and resume cleanly · matches land with validated precision against a hand-labeled set, on at least one real brief · tired-feed flag fires correctly.
...
- **Deeper audio:** speaker-aware / multi-lingual transcript handling (the basic transcript tier already ships in v1).
...
## 12. Open questions
- Exact session/daily caps — discover empirically; kept conservative given hostile bot detection.
- Local model sizes (text + vision + audio all resident-capable?) — measure real free memory on the M4 first; audio adds pressure.
- The video + comment endpoints (and any signature/anti-bot headers in the captured traffic) — identify once in DevTools; expect frequent drift.
...
| Risk | Mitigation |
|---|---|
| Account ban (aggressive bot detection) | read-only · careful mobile warming · CDP-attach (real-not-vanilla browser) · conservative human-paced ramp · halt-on-resistance |
| Endpoint / signature drift breaks scraping | intercept by URL + response shape, not hardcoded IDs; empty-interception canary halts |
| Relevance judged half-blind (thin captions) | vision/OCR **and** transcript read in **v1** + escalate-if-unsure to cloud |
| Local model weak on Uzbek | cloud escalation on both gate and scoring + hand-labeled validation set |
| Feed runs dry / drifts off-niche | already-seen-skip counter → dashboard flag → operator re-steers on mobile |
| Desktop can't steer the feed | steering done manually on mobile (recurring); accepted maintenance cost |
| Audio tier oversubscribes memory | load Whisper on-demand, unload after batch |
