# Instagram Reel-Comment Discovery Agent — PRD (v1 & v2)

**Status:** Draft v0.3 · **Date:** 2026-06-10 · *product name TBD*
...
**Changes since v0.2:** (a) "look at the video" (on-screen text via vision/OCR) moved into v1; (b) relevance gate now escalates to cloud when unsure, same as the comment cascade; (c) added a tired-feed detector (too many already-seen reels → dashboard flag); (d) documented that feed steering is manual, on mobile, and recurring.

---

## 1. Summary
A local-first agent that discovers relevant Reels through Instagram's algorithmic feed, **reads both the caption and the on-screen text of the video**, then reads the comments and scores/extracts them against a **campaign-defined brief**, surfacing matches in an admin panel for human follow-up. The engine is domain- and goal-agnostic and must run against a brief it has never seen with zero code change. Runs on the operator's own Mac in short daytime bursts — not on a server, not 24/7.
...
**Goals**
- Surface brief-matching comments from public Reels, where relevance, goal type, what-to-extract, and threshold all live in `campaign.md`.
- Survive on Instagram — behave like a real human account, never trip automation enforcement.
...
- The bot never likes, follows, comments, or DMs. Dwell + scroll only.
...
- **Feed steering is manual + mobile + recurring** — desktop watching does not meaningfully train the algorithm; the operator steers the feed by hand on mobile during warming and re-nudges it periodically as it drifts (see §10).
...
- **campaign.md** — warming/seed direction, relevance definition, goal type, what to extract & score, threshold, language mix.
...
```
- **Engine:** Python + Playwright over CDP. Walks the feed, reads caption + on-screen text, intercepts JSON, runs the cascade against the active brief, writes matches + state.
- **Model router:** call sites — `classifyText`, `classifyImage` (on-screen text / relevance), `transcribe` (v2) — each routes per-tier to local or OpenRouter.
...
|---|---|---|---|
| Relevance gate | is this reel relevant to the brief? | Local text → **local vision (reads on-screen text)** → **escalate-if-unsure → cloud** | the harder, irreversible decision now has a backup |
...
| Audio (v2) | voiceover transcript | Local (Whisper) | rare, runs last |

One model resident at a time (36GB cap). **Batch by stage** — run the text gate across reels, then load vision once for the batch that needs it, unload, then escalate the unsure minority to cloud. Cloud tier needs fallback (retry queue or degrade-to-local + flag) and per-campaign spend logging.

## 7. State model (the spine)
Persisted in SQLite between sessions:
- **seen_reels** — dedupe; forward-only watermark.
- **comment cursors** — per reel; "new comments since last poll."
- **watchlist** — match-rich reels, re-polled until aged out (~7–14 days).
- **session counters** — reels seen, **already-seen skips**, relevance passes, matches, escalations, spend.
...
`campaign_id, reel_id, comment_id, username, text, lang, score, reason, extracted, status, captured_at`
- `score` / `reason` — interpreted per the active brief.
- `extracted` — brief-defined JSON (lead → phone/intent; partner → reel author; signal → topic).
- `status` ∈ { new, reviewing, confirmed, discarded }

## 9. Failure handling (three tiers)
- **Auto-skip + continue:** transient (reel won't load, single parse fails).
...
- **Halt + alert human (stop session):** login expired, checkpoint/captcha/challenge, action-block, or empty interception for N reels (endpoint drift). The bot never attempts to resolve a checkpoint.

## 10. Pacing & feed steering
- Weeks 1–2: manual warming only, no automation.
- **Steering is ongoing, on mobile.** The operator watches the right content by hand on the phone during warming and re-nudges periodically; the agent's desktop watching does not steer the feed. Expect a recurring few-minutes maintenance task, not one-time setup. A rising already-seen-skip ratio (§7) is the signal to go re-steer.
- Agent runs: ramp from low — ~1–2 sessions/day, 15–30 min, 20–40 reels; dwell 3–30s, between-reel 2–8s, randomized; daytime only. Ramp until resistance, then hold below it. Caps discovered empirically.
...
- Manual account creation + 1–2 week human warming; ongoing mobile re-steering.
- CDP attach to warmed Chrome.
- Feed-discovery loop: dwell-on-relevant / scroll-past-irrelevant.
- **Relevance gate: caption text → on-screen-text (vision/OCR) → escalate-if-unsure to cloud.**
- Interception of reels + comments; top-level comments; reply-expansion only on matching comments.
- Comment cascade: local pre-filter → local scoring → escalate-if-unsure to cloud.
- **Vision/OCR tier (reads on-screen text).**
...
- **Audio tier:** Whisper transcription of voiceover.
...
## 12. Open questions
- Exact session/daily caps — discover empirically.
- Local model sizes (text + vision both resident-capable?) — measure real free memory on the M4 first.
- The comment endpoint — identify once in DevTools; expect drift.
...
| Risk | Mitigation |
|---|---|
| Account ban | read-only · manual warming · CDP-attach · human-paced ramp · halt-on-resistance |
| Endpoint drift breaks scraping | intercept by URL + response shape, not hardcoded IDs; empty-interception canary halts |
| Relevance judged half-blind (caption only) | vision/OCR reads on-screen text **in v1** + escalate-if-unsure to cloud |
